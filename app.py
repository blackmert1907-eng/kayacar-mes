...
DB = Path(os.getenv("KAYACAR_DB_PATH", str(APP_DIR / "kayacar_v02.db")))
...
.block-container{padding-left:.6rem!important;padding-right:.6rem!important;padding-top:.45rem!important}
h1{font-size:1.5rem!important} h2{font-size:1.2rem!important} h3{font-size:1.05rem!important}
.stButton>button{min-height:46px;width:100%;font-size:.95rem}
[data-testid="stMetric"]{padding:.6rem!important}
[data-testid="stMetricValue"]{font-size:1.2rem!important}
[data-testid="stDataFrame"]{font-size:.78rem!important}
div[data-testid="stTabs"] button{padding-left:.55rem!important;padding-right:.55rem!important;font-size:.8rem!important}
.kc-step{font-size:1rem!important;padding:10px!important}
...
if os.getenv("KAYACAR_CLOUD_MODE","0")=="1":
    st.info("📱 Mobil/Cloud Modu aktif — telefondan tarayıcıyla kullanılabilir.")

if "user" not in st.session_state:
    st.session_state["user"]=None

if not st.session_state["user"]:
    st.title("⚗️ KayaCar Kimyasal MES")
    st.caption("Giriş yap")
    with st.form("login"):
        u=st.text_input("Kullanıcı")
        p=st.text_input("Şifre",type="password")
        go=st.form_submit_button("Giriş",type="primary")
    if go:
    ...
    st.info("İlk kurulum: admin / 1234 veya operator / 1234. İlk girişten sonra şifreleri değiştir.")
    st.stop()
...
top1.title("⚗️ KayaCar Kimyasal — MES")
top1.caption(f"v0.9 • Kullanıcı: {USER['username']} • Rol: {ROLE}")
if top2.button("Çıkış"):
...
if ROLE=="OPERATOR":
    ...
    tabs=st.tabs(["📊 Patron Paneli","🏭 Üretim Konsolu","🧾 Reçeteler","📦 Hammadde Stok","🧾 Satın Alma / Tank","🛢️ Tanklar","📦 Mamul / WIP","⚖️ Terazi Ayarı","🏷️ Parti / QR","🗂️ Parti Geçmişi","👥 Kullanıcı / Log"])
...
# ---------------- ADMIN UI ----------------
    ...
    with tabs[0]:
        ...
        st.caption('v0.9: üretim yürütme motoru hammadde-satırı değil, FAZ/OPERASYON mantığıyla çalışır.')
    ...
    with tabs[2]:
        ...
        with st.expander("🏗️ v0.9 FAZ MİMARİSİ / ISA-88 BENZERİ YÜRÜTME", expanded=True):
    ...
    # ---- QR / Label ----
    with tabs[8]:
        st.subheader("Parti QR ve etiket")
        hist=all_batches()
        if hist.empty:
            st.info("Parti yok.")
        else:
            qsel=st.selectbox("Parti",hist["batch_no"].tolist(),key="qr_batch")
            bq=get_batch(qsel)
            st.code(batch_label_text(qsel))
            png=qr_png_bytes(qsel)
            if png:
                st.image(png,width=240)
                st.download_button("QR PNG indir",png,file_name=f"{qsel}_QR.png",mime="image/png")
            else:
                st.warning("qrcode paketi kurulu değil.")
            st.download_button("Etiket TXT indir",batch_label_text(qsel).encode("utf-8"),
                               file_name=f"{qsel}_etiket.txt",mime="text/plain")
    ...
    with tabs[8]:
        ...
        if not hist.empty:
            sel=st.selectbox("Parti detayı",hist["batch_no"].tolist())
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.9 — Faz tabanlı batch motoru: FAZ→DOZAJLAR→FAZ KARIŞTIRMA/KONTROL→SONRAKİ FAZ; proses suyu ve kütle dengesi görünür.")
