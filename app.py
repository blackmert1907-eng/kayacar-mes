...
def render_brand_header(login=False):
    """Pure HTML wordmark so the mobile header can never crop the logo image."""
    compact = not login
    pad = "16px 18px" if compact else "22px 22px"
    word = "42px" if compact else "58px"
    sub = "11px" if compact else "13px"
    margin = "6px" if compact else "12px"
    st.markdown(f"""
    <div class="kc-brandbar" style="padding:{pad};margin-bottom:{margin}">
      <div class="kc-wordmark" style="font-size:{word}">
        <span class="kc-kaya">Kaya</span><span class="kc-car">Car</span>
      </div>
      <div class="kc-tag" style="font-size:{sub}">PROFESSIONAL CAR CARE CHEMICALS</div>
...
def generate_front_label_png(product,batch_no="",net_text="1 L",product_image_bytes=None):
    ...
    white=(247,249,250); gray=(174,184,191); ac=_hex_rgb(accent)
    d.rounded_rectangle((30,30,W-30,H-30),radius=38,outline=(61,76,87),width=3,fill=(7,12,17))
    d.rounded_rectangle((62,60,W-62,76),radius=8,fill=ac)

    # clean wordmark, never tiny
    d.text((65,112),"Kaya",font=_font(68,True),fill=white)
    kw=d.textbbox((65,112),"Kaya",font=_font(68,True))[2]-65
    d.text((65+kw,112),"Car",font=_font(68,True),fill=ac)
    d.text((68,194),"PROFESSIONAL CAR CARE CHEMICALS",font=_font(22,False),fill=gray)
    d.line((65,245,W-65,245),fill=(65,80,90),width=2)

    # Product image / visual
    photo_box=(600,300,915,620)
    pasted=_paste_product_image(img,product_image_bytes,photo_box)
    if not pasted:
        if "Lastik" in product:
            _draw_tire_visual(d,photo_box,accent)
        else:
            d.rounded_rectangle(photo_box,radius=34,outline=ac,width=4,fill=(12,20,26))
            d.text((676,386),"KayaCar",font=_font(32,True),fill=white)
            d.text((694,438),"PRO",font=_font(24,True),fill=ac)
            d.text((646,490),"CAR CARE",font=_font(28,True),fill=gray)

    d.text((65,322),en,font=_font(28,False),fill=gray)
    y=372
    for line in _wrap(d,tr,_font(49,True),510):
        d.text((65,y),line,font=_font(49,True),fill=white)
        y+=58
    y+=20
    for line in _wrap(d,claim,_font(24,True),510):
        d.text((65,y),line,font=_font(24,True),fill=white); y+=34

    # Features - compact, readable, not empty
    copy=label_copy(product)
    y=700
    for i,txt in enumerate(copy["icons"][:4]):
        row=i//2; col=i%2
        x=65+col*445; yy=y+row*82
        d.rounded_rectangle((x,yy,x+405,yy+58),radius=16,outline=ac,width=2,fill=(13,22,29))
        d.text((x+18,yy+16),txt,font=_font(20,True),fill=white)

    # Use instruction snippet
    d.text((65,905),"KULLANIM / DIRECTIONS",font=_font(23,True),fill=ac)
    snippet=copy["tr_use"]
    for line in _wrap(d,snippet,_font(21,False),850)[:4]:
        d.text((65,945),line,font=_font(21,False),fill=white); 
        # increment manually based on current last
        snippet = snippet
        break
    lines=_wrap(d,copy["tr_use"],_font(21,False),850)[:4]
    yy=945
    for line in lines:
        d.text((65,yy),line,font=_font(21,False),fill=white); yy+=29

    # Footer
    d.line((65,H-245,W-65,H-245),fill=(65,80,90),width=2)
    d.text((65,H-215),"PROFESSIONAL FORMULA",font=_font(27,True),fill=white)
    d.text((65,H-174),codep,font=_font(21,False),fill=gray)
    nb=d.textbbox((0,0),net_text,font=_font(54,True))
    d.text((W-65-(nb[2]-nb[0]),H-215),net_text,font=_font(54,True),fill=white)
    d.text((65,H-102),f"BATCH / LOT: {lot}",font=_font(20,False),fill=gray)
...
def generate_back_label_png(product,batch_no="",net_text="1 L"):
    ...
    d.rounded_rectangle((30,30,W-30,H-30),radius=38,outline=(61,76,87),width=3,fill=(7,12,17))
    d.rounded_rectangle((62,60,W-62,76),radius=8,fill=ac)
    d.text((65,105),"Kaya",font=_font(48,True),fill=white)
    kw=d.textbbox((65,105),"Kaya",font=_font(48,True))[2]-65
    d.text((65+kw,105),"Car",font=_font(48,True),fill=ac)
    d.text((65,168),f"{tr}  |  {net_text}",font=_font(23,True),fill=gray)
    d.line((65,215,W-65,215),fill=(65,80,90),width=2)

    sections=[
      ("TR - KULLANIM", copy["tr_use"]),
      ("TR - UYARILAR", "\n".join("- "+x for x in copy["tr_warn"])),
      ("EN - DIRECTIONS", copy["en_use"]),
      ("EN - WARNINGS", "\n".join("- "+x for x in copy["en_warn"])),
      ("DE - ANWENDUNG", copy["de_use"]),
      ("DE - WARNHINWEISE", "\n".join("- "+x for x in copy["de_warn"])),
    ]
    y=255
    body=_font(18,False); head=_font(20,True)
    for title,txt in sections:
        d.text((65,y),title,font=head,fill=ac); y+=29
        paragraphs=str(txt).split("\n")
        for para in paragraphs:
            lines=_wrap(d,para,body,860)
            for line in lines:
                d.text((65,y),line,font=body,fill=white); y+=24
            y+=3
        y+=13
        if y>1180:
            break

    d.line((65,H-170,W-65,H-170),fill=(65,80,90),width=2)
    footer=[
        f"Ürün Kodu / Product Code: {codep}",
        f"Parti / Lot: {lot}",
        "Üretim Tarihi / MFG: ____ / ____ / ______    SKT/TETT: ____ / ____ / ______",
        "KayaCar Kimyasal - Professional Car Care Chemicals"
    ]
    fy=H-142
    for i,line in enumerate(footer):
        d.text((65,fy+i*27),line,font=_font(17,True if i==3 else False),fill=white if i==3 else gray)
...
with head:
    ...
    st.caption(f"v0.13 MOBILE PRO • Kullanıcı: {USER['username']} • Rol: {ROLE}")
with logout:
    st.write("")
...
global_q=st.text_input("🔎 Uygulamada ara",placeholder="Ürün, hammadde, tank/varil, parti no, kullanıcı...",key="global_search_v013")
if global_q.strip():
...
# ---------------- ADMIN UI ----------------
    ...
    with tabs[8]:
        ...
        st.caption("v0.13: mobil okunabilir etiket motoru. Uyarı metinleri taslaktır; ticari satıştan önce ürünün SDS/SEA/CLP sınıflandırması ve hedef ülke etiket mevzuatıyla doğrulanmalıdır.")
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.13 MOBILE PRO — Global arama • Faz/ön karışım kapları • Ayrı onay PIN’i • Tank/varil yönetimi • Mobil ön/arka etiket PNG/PDF • QR • Audit.")
