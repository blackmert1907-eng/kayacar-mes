...
def init_db():
    ...
    # v0.15 migration: old "Yağ Fazı" card -> user-facing "Yağ Fazı".
    old_oil=con.execute("SELECT stock_kg,unit_cost_tl,supplier,lot_no FROM raw_materials WHERE name='Yağ Fazı'").fetchone()
    new_oil=con.execute("SELECT stock_kg,unit_cost_tl FROM raw_materials WHERE name='Yağ Fazı'").fetchone()
    if old_oil and float(old_oil["stock_kg"] or 0)>0:
        ...
        con.execute("""UPDATE raw_materials SET stock_kg=?,unit_cost_tl=?,
                     supplier=COALESCE(NULLIF(supplier,''),?),lot_no=COALESCE(NULLIF(lot_no,''),?)
                     WHERE name='Yağ Fazı'""",
                    (total,avg,old_oil["supplier"] or "",old_oil["lot_no"] or ""))
        con.execute("UPDATE raw_materials SET stock_kg=0 WHERE name='Yağ Fazı'")
...
def deduct_stock(material,actual_g,ref=""):
    ...
    if float(current["stock_kg"])+1e-9 < kg:
        con.close()
        raise ValueError(f"Yetersiz stok: {material}. Mevcut: {float(current['stock_kg']):.3f} kg • Hammadde Stok / Satın Alma bölümünden giriş yapın.")
...
def render_phase_engine(active,operator=False):
    ...
    # v0.15: Yağ Fazı kaynağı her durumda görünür.
    # Böylece v0.13/v0.14'te zaten DOSE aşamasına geçmiş eski partiler de tıkanmaz.
    if stage.get("title")=="Yağ Fazı":
        current_oil_source=batch_param_get(active,f"oil_phase_source_{sidx}") or "COMPONENTS"
        oil_choices=["HAMMADDELERDEN HAZIRLA","HAZIR YAĞ FAZI STOĞUNDAN KULLAN"]
        oil_idx=1 if current_oil_source=="PREMIX" else 0
        chosen=st.radio("🧪 Yağ Fazı Kaynağı",oil_choices,index=oil_idx,
                        key=f"oilsource_live_{active}_{sidx}",horizontal=False)
        desired="PREMIX" if chosen.startswith("HAZIR") else "COMPONENTS"
        if desired!=current_oil_source:
            # Source change is only safe before an oil-phase dose has been logged.
            con=db()
            ...
            con.close()
            if any_dose:
                st.error("Bu Yağ Fazı aşamasında dozaj kaydı başladığı için kaynak değiştirilemez.")
            else:
                batch_param_set(active,f"oil_phase_source_{sidx}",desired,
                                "OPERATÖR KAYNAK DEĞİŞİKLİĞİ",USER["username"])
                set_stage_state(active,"STAGE_DOSING",sidx,0)
                stage_event(active,sidx,stage["title"],"OIL_PHASE_SOURCE_CHANGE",
                            "Hazır Yağ Fazı stoğu" if desired=="PREMIX" else "Bileşenlerden hazırlama",
                            USER["username"])
                st.rerun()

        if desired=="PREMIX":
            oil_target=sum(total_formula_g(product,target,m) for m in stage.get("materials",[]))
            st.info(f"📦 Yağ Fazı ana stok: {material_stock('Yağ Fazı'):.3f} kg • Bu parti hedefi: {oil_target:.2f} g")
        else:
            st.caption("Bu seçimde Polybutene + Silikon Yağı + DPM ayrı ayrı stoktan düşülür.")
        mats=phase_materials(product,stage,active,target)
    ...
    if state=="STAGE_INTRO":
        ...
        if stage.get("title")=="Yağ Fazı":
            ...
            if oil_choice.startswith("HAZIR"):
                st.info(f"Yağ Fazı ana stok: {material_stock('Yağ Fazı'):.3f} kg")
...
with head:
    ...
    st.caption(f"v0.15 MOBILE PRO • Kullanıcı: {USER['username']} • Rol: {ROLE}")
with logout:
    st.write("")
...
# ---------------- ADMIN UI ----------------
    ...
    if NAV=="🧾 Satın Alma / Tank":
        ...
        st.caption("Yağ Fazı dahil tüm stok kartları burada aranabilir. Hazır Yağ Fazı kullanacaksanız satın alma/üretim stok girişini 'Yağ Fazı' kartına yapın.")
    ...
    if NAV=="🏷️ Etiketleme / QR":
        ...
        st.caption("v0.15: mobil okunabilir etiket motoru. Uyarı metinleri taslaktır; ticari satıştan önce ürünün SDS/SEA/CLP sınıflandırması ve hedef ülke etiket mevzuatıyla doğrulanmalıdır.")
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.15 MOBILE PRO — Yağ Fazı stok kartı ve eski parti kurtarma • Canlı kaynak seçimi • Ayrı onay PIN’i • Mobil üretim • Etiket PNG/PDF • Audit.")
