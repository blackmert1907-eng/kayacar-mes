...
def mobile_table(df,max_rows=100):
    ...
    st.markdown(
        "<div class='mobile-table-wrap' style='background:#0b1118;color:#eef4f7'>"
        + html +
        "</div>",
...
def render_phase_engine(active,operator=False):
    ...
    if consumers:
        for producer_title,crule in consumers:
            ...
            if q<=0.001:
                st.error(f"⛔ **{wname} hazır değil.** Önce “{producer_title}” fazında ara ürünü üretip tamamlamalısın.")
            else:
...
with head:
    ...
    st.caption(f"v0.19 MOBILE PRO • Kullanıcı: {USER['username']} • Rol: {ROLE}")
with logout:
    st.write("")
...
# ---------------- ADMIN UI ----------------
    ...
    if NAV=="🏷️ Etiketleme / QR":
        ...
        st.caption("v0.19: mobil okunabilir etiket motoru. Uyarı metinleri taslaktır; ticari satıştan önce ürünün SDS/SEA/CLP sınıflandırması ve hedef ülke etiket mevzuatıyla doğrulanmalıdır.")
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.19 MOBILE PRO — Mobil koyu tema tablo/form düzeltmesi • Ara ürün/ön karışım motoru • WIP transferi • Stok • Etiket • Audit.")
