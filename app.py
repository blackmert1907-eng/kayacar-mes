...
def init_db():
    ...
    all_mats += ["Yağ Fazı"]
    # Only intermediates deliberately allowed as reusable stock cards.
    all_mats += sorted({r["output"] for r in INTERMEDIATE_RULES.values() if r.get("stockable")})
    all_mats = sorted(set(all_mats))
    for m in all_mats:
        con.execute("INSERT OR IGNORE INTO raw_materials(name) VALUES(?)",(m,))

    # v0.16 safe migration:
    old_oil=con.execute(
        "SELECT stock_kg,unit_cost_tl,supplier FROM raw_materials WHERE name='Yağ Fazı Premix'"
    ).fetchone()
    new_oil=con.execute(
        "SELECT stock_kg,unit_cost_tl,supplier FROM raw_materials WHERE name='Yağ Fazı'"
    ).fetchone()
    if old_oil and float(old_oil["stock_kg"] or 0)>0:
        old_qty=float(old_oil["stock_kg"] or 0)
        old_cost=float(old_oil["unit_cost_tl"] or 0)
        new_qty=float(new_oil["stock_kg"] or 0) if new_oil else 0.0
        new_cost=float(new_oil["unit_cost_tl"] or 0) if new_oil else 0.0
        total=new_qty+old_qty
        avg=((new_qty*new_cost)+(old_qty*old_cost))/total if total>0 else old_cost
        supplier=(new_oil["supplier"] if new_oil and new_oil["supplier"] else old_oil["supplier"]) or ""
        con.execute(
            "UPDATE raw_materials SET stock_kg=?,unit_cost_tl=?,supplier=? WHERE name='Yağ Fazı'",
            (total,avg,supplier)
        )
        con.execute("UPDATE raw_materials SET stock_kg=0 WHERE name='Yağ Fazı Premix'")
...
def deduct_stock(material,actual_g,ref=""):
    ...
    if float(current["stock_kg"])+1e-9 < kg:
        con.close()
        raise ValueError(
        f"Yetersiz stok: {material}. Mevcut: {float(current['stock_kg']):.3f} kg • "
        f"Hammadde Stok / Satın Alma bölümünde '{material}' kartına giriş yapın."
    )
    con.execute("UPDATE raw_materials SET stock_kg=stock_kg-? WHERE name=?",(kg,material))
    tank=con.execute("""SELECT id,current_kg FROM tanks
                       WHERE material=? AND current_kg>=?
                       AND COALESCE(active,1)=1
                       AND COALESCE(tank_type,'RAW_MATERIAL')='RAW_MATERIAL'
...
INTERMEDIATE_RULES = {
    ...
    ("Pembe Oto Yıkama Şampuanı","Boya Ön Çözeltisi"): {
        "output":"Boya Ön Çözeltisi",
        "consume_stage":"Renklendirme",
        "stockable":False,
        "instruction":"Boyayı küçük miktar deiyonize suyla ayrı kapta ön çözelt; topak/çökelti bırakma."
    },
    ("Zift / Katran Sökücü","Solvent Fazı"): {
        "output":"Solvent Fazı",
        "consume_stage":"Birleştirme",
        "stockable":False,
        "instruction":"D-Limonene + DPM + Noniyonik Emülgatörü ayrı kapta homojen ön karışım haline getir."
    },
    ("Oto Parfümü - Water Based Spray","Ön Karışım"): {
        "output":"Parfüm Ön Karışımı",
        "consume_stage":"Etanol",
        "stockable":False,
        "instruction":"Esans + PEG-40 HCO + DPM'yi ayrı kapta homojen/berrak ön karışım haline getir."
    },
    ("Oto Parfümü - Water Based Spray","Etanol"): {
        "output":"Koku Konsantresi",
        "consume_stage":"Birleştirme",
        "stockable":False,
        "input_wip":"Parfüm Ön Karışımı",
        "instruction":"Hazırlanmış Parfüm Ön Karışımına Etanol %96'yı yavaşça ekle; homojen Koku Konsantresi oluştur."
    },
}
...
def phase_materials(product,stage,batch_no,target_kg):
    ...
    text=(stage.get("source_text") or "").lower()
    b=get_batch(batch_no) or {}
    sidx=int(b.get("stage_index") or 0)

    # Producer stage: either make the intermediate now from components or use ready stock.
    rule=intermediate_rule(product,title)
    if rule:
        mode=premix_mode(batch_no,sidx)

        # Chained producer may first receive a WIP from the prior stage.
        input_wip=rule.get("input_wip")
        if input_wip:
            mats=[virtual_wip_material(input_wip)] + mats

        if mode=="READY_STOCK" and rule.get("stockable"):
            mats=[rule["output"]]

    # Consumer/transfer stage: prepared intermediate is a real required operation,
    # but it is NOT deducted from raw-material stock again.
    producers=producer_for_consume_stage(product,title)
    for producer_title,prule in producers:
        wname=prule["output"]
        vmat=virtual_wip_material(wname)
        if vmat not in mats:
            mats.insert(0,vmat)
...
def phase_target_g(batch_no,product,target_kg,stage,material):
    wname=virtual_wip_name(material)
    if wname:
        ready=wip_qty(batch_no,wname)
        if ready>0:
            return ready

        # If the WIP is expected but missing, use expected producer quantity only
        # for display. Confirmation remains blocked by wip_consume().
        producers=producer_for_consume_stage(product,stage.get("title",""))
        for producer_title,rule in producers:
            if rule["output"]==wname:
                legacy=legacy_intermediate_dose(active,sidx,wname)
                if legacy:
                    st.info(
                        f"Eski sürümden bu fazda **{wname} {float(legacy['actual_g']):.2f} g** dozaj kaydı bulundu. "
                        "Bu kayıt yeni WIP motoruyla aynı şey değildir; yeni test partisinde ara ürün akışını baştan denemen daha sağlıklı."
                    )
                pidx=stage_index_by_title(product,producer_title)
                if pidx is not None:
                    pst=phase_list(product)[pidx]
                    return expected_stage_components_g(product,target_kg,pst)
        return 0.0

    rule=intermediate_rule(product,stage.get("title"))
    if rule and material==rule.get("output"):
        # Ready intermediate stock replaces producer components.
        return expected_stage_components_g(product,target_kg,stage)
...
def confirm_phase_dose(batch_no,stage_index,material_index,material,target_g,actual_g,source,vessel):
    ...
    if phase_material_dosed(batch_no,stage_index,material_index):
        raise ValueError("Bu faz hammaddesi / ara ürün daha önce kaydedildi.")
    tol=max(float(target_g)*(float(setting("tolerance_pct","0.50"))/100),
            float(setting("min_tolerance_g","1.0")))

    wname=virtual_wip_name(material)
    if wname:
        # Same-batch intermediate: no raw stock deduction.
        wip_consume(batch_no,wname,actual_g)
        movement_source="ARA_URUN_TRANSFER"
    else:
        deduct_stock(material,actual_g,ref=batch_no)
    ...
    con.execute("""INSERT INTO dosing_log(
                   ...
                   VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                (bid,int(seq),material,float(target_g),float(actual_g),float(tol),movement_source,
...
def premix_return_key(producer_stage_index):
    return f"premix_return_stage_{int(producer_stage_index)}"

def send_batch_to_premix_producer(batch_no,producer_stage_index,return_stage_index):
    """Temporarily jump back to the producer stage, then return to the blocked consumer."""
    b=get_batch(batch_no)
    if not b:
        raise ValueError("Parti bulunamadı.")
    product=b["product"]
    phases=phase_list(product)
    if producer_stage_index<0 or producer_stage_index>=len(phases):
        raise ValueError("Ara ürün üretim fazı bulunamadı.")

    # Save where we must return after the premix is ready.
    batch_param_set(
        batch_no,
        premix_return_key(producer_stage_index),
        str(int(return_stage_index)),
        "ARA ÜRÜN GERİ DÖNÜŞ",
        USER["username"]
    )
    ...
    con.execute("""UPDATE production_batches
                   ...
                   WHERE batch_no=?""",
                (int(producer_stage_index),datetime.now().isoformat(timespec="seconds"),batch_no))
    con.commit(); con.close()
    stage_event(batch_no,producer_stage_index,phases[producer_stage_index]["title"],
                "PREMIX_REWORK_START",
                f"Ara ürün şimdi hazırlanacak; tamamlanınca faz {return_stage_index+1}'e dön.",
                USER["username"])
    audit(USER["username"],"PREMIX_REWORK_START",
          f"{batch_no} | producer={producer_stage_index} | return={return_stage_index}")

def stage_has_any_dose(batch_no,stage_index):
    con=db()
    row=con.execute("""SELECT 1
                       FROM dosing_log dl
                       JOIN production_batches pb ON pb.id=dl.batch_id
                       WHERE pb.batch_no=? AND dl.stage_index=?
                       LIMIT 1""",(batch_no,int(stage_index))).fetchone()
    con.close()
    return bool(row)


def legacy_intermediate_dose(batch_no,stage_index,material_name):
    con=db()
    row=con.execute("""SELECT dl.actual_g,dl.id
                       FROM dosing_log dl
                       JOIN production_batches pb ON pb.id=dl.batch_id
                       WHERE pb.batch_no=? AND dl.stage_index=? AND dl.material=?
                       ORDER BY dl.id DESC LIMIT 1""",
                    (batch_no,int(stage_index),material_name)).fetchone()
    con.close()
    return dict(row) if row else None
...
def advance_stage(batch_no):
    ...
    product=b["product"]
    phases=phase_list(product)
    current_idx=int(b.get("stage_index") or 0)

    # v0.20: if this stage was entered only to prepare a missing intermediate,
    # return directly to the blocked consumer stage after WIP is ready.
    return_raw=batch_param_get(batch_no,premix_return_key(current_idx),"")
    if str(return_raw).strip().isdigit():
        nxt=int(return_raw)
        batch_param_set(batch_no,premix_return_key(current_idx),"",
                        "ARA ÜRÜN GERİ DÖNÜŞ TAMAMLANDI",USER["username"])
    else:
        nxt=current_idx+1
    if nxt>=len(phases):
        con=db()
...
def render_phase_engine(active,operator=False):
    ...
    # v0.18 — show current batch-scoped intermediates
    isum=intermediate_summary(active,product)
    if not isum.empty:
        with st.expander("🧪 Parti Ara Ürünleri / Ön Karışımlar",expanded=False):
            mobile_table(isum)

    # General intermediate producer selector
    irule=intermediate_rule(product,stage.get("title"))
    if irule:
        st.markdown(
            f"""<div style="background:#0d2030;border:1px solid #2ca9d0;border-radius:14px;
                 padding:15px 16px;margin:10px 0">
                 <div style="font-size:.78rem;color:#74ccea;font-weight:700">ARA ÜRÜN / ÖN KARIŞIM</div>
                 <div style="font-size:1.25rem;color:white;font-weight:800;margin-top:3px">
                 🧪 {irule['output']} HAZIRLA</div>
                 <div style="color:#c6d4dc;margin-top:7px">{irule['instruction']}</div>
                 </div>""",
            unsafe_allow_html=True
        )
        current_mode=premix_mode(active,sidx)
        if irule.get("stockable"):
            choices=["ŞİMDİ ÜRET — HAMMADDELERDEN HAZIRLA","HAZIR ARA STOKTAN KULLAN"]
            idx=1 if current_mode=="READY_STOCK" else 0
            choice=st.radio(f"🧪 {irule['output']} kaynağı",choices,index=idx,
                            key=f"premix_source_{active}_{sidx}")
            desired="READY_STOCK" if choice.startswith("HAZIR") else "MAKE_NOW"
        else:
            desired="MAKE_NOW"
            st.info(f"🧪 **{irule['output']}** bu parti içinde anlık hazırlanacak; kalıcı hammadde stoğu olarak aranmayacak.")

        if desired!=current_mode:
            con=db()
            any_dose=con.execute("""SELECT 1 FROM dosing_log dl
                                    JOIN production_batches pb ON pb.id=dl.batch_id
                                    WHERE pb.batch_no=? AND dl.stage_index=? LIMIT 1""",
                                 (active,sidx)).fetchone()
            con.close()
            if any_dose:
                st.error("Bu ara ürün fazında dozaj kaydı başladığı için kaynak değiştirilemez.")
            else:
                set_premix_mode(active,sidx,desired,stage.get("title"))
                set_stage_state(active,"STAGE_INTRO",sidx,0)
                stage_event(active,sidx,stage["title"],"INTERMEDIATE_SOURCE_CHANGE",
                            desired,USER["username"])
                st.rerun()

        if desired=="READY_STOCK":
            need=expected_stage_components_g(product,target,stage)
            st.info(f"📦 Hazır **{irule['output']}** stoğu: {material_stock(irule['output']):.3f} kg • Parti ihtiyacı: {need:.2f} g")
        else:
            st.success(f"➡️ Şimdi üret akışı: {irule['instruction']}")
        mats=phase_materials(product,stage,active,target)

    # Consumer stage: make sure required WIP exists before operator tries to transfer it.
    consumers=producer_for_consume_stage(product,stage.get("title"))
    if consumers:
        for producer_title,crule in consumers:
            wname=crule["output"]
            q=wip_qty(active,wname)
            if q<=0.001:
                st.error(f"⛔ **{wname} hazır değil.** Bu faz devam etmeden önce ara ürün hazırlanmalı.")
                pidx=stage_index_by_title(product,producer_title)
                if pidx is not None:
                    st.markdown(
                        f"""<div style="background:#171b21;border:1px solid #d09b2c;border-radius:12px;
                             padding:13px;margin:8px 0">
                             <b>Gerekli ara ürün:</b> {wname}<br>
                             <b>Hazırlama fazı:</b> {producer_title}<br>
                             <span style="color:#b9c2c8">{crule.get('instruction','')}</span>
                             </div>""",
                        unsafe_allow_html=True
                    )
                    # Only permit a clean rewind before the current consumer has real dose records.
                    if not stage_has_any_dose(active,sidx):
                        if st.button(
                            f"🧪 {wname.upper()} — ŞİMDİ HAZIRLA",
                            type="primary",
                            use_container_width=True,
                            key=f"make_missing_wip_{active}_{sidx}_{pidx}"
                        ):
                            send_batch_to_premix_producer(active,pidx,sidx)
                            st.rerun()
                    else:
                        st.warning(
                            "Bu fazda daha önce dozaj kaydı oluşmuş. İzlenebilirlik için otomatik geri sarma kilitli. "
                            "Test partisiyse yeni parti başlatmak en temiz yöntemdir."
                        )
            else:
                st.success(f"✅ Hazır ara ürün: **{wname} {q:.2f} g** • Bu fazda ana kazana transfer edilecek.")
        mats=phase_materials(product,stage,active,target)

    stage_mats=list(stage.get("materials",[]))
    if stage.get("title")=="Emülsifikasyon":
        st.info("ℹ️ Emülsifikasyon bir stok kalemi değildir. Önce hazırlanmış **Yağ Fazı** ana kazana transfer edilir; ardından Noniyonik Emülgatör ile proses yürütülür.")
    elif stage_mats:
        st.caption("📦 Bu fazın ham madde girdileri: " + ", ".join(stage_mats))
    ...
    if state=="STAGE_INTRO":
        ...
        if st.button("✅ FAZ HAZIR — DEVAM",type="primary",use_container_width=True,key=f"stageintro_{active}_{sidx}"):
            ...
            stage_event(active,sidx,stage["title"],"STAGE_START",
                        stage.get("source_text",""),USER["username"])
            st.rerun()
        return
    ...
    if state=="STAGE_DOSING" and mats:
        ...
        with c1:
            ...
            wname=virtual_wip_name(material)
            if wname:
                st.info(f"🧪 Parti ara ürünü: {wname} • Hazır {wip_qty(active,wname):.2f} g")
            else:
        ...
        with c3:
            ...
            wip_missing=bool(virtual_wip_name(material) and wip_qty(active,virtual_wip_name(material))<=0.001)
            if wip_missing:
                st.error("Ara ürün henüz üretilmedi / hazır değil. Önce ilgili ön karışım fazını tamamla.")
            if st.button("✅ DOZAJI KAYDET",type="primary",
                         disabled=(wip_missing or not(actual>0 and (ok or override))),
        ...
        if kind in ("CONTROL","QC","TRANSFER"):
            ...
            if kind=="CONTROL" and ("ph" in stage.get("title","").lower() or "ph" in stage.get("source_text","").lower()):
                ...
                if st.button("✅ FAZI ONAYLA — SONRAKİ FAZ",type="primary",
                    ...
                    finalize_stage_intermediate(active,product,target,sidx,stage)
...
def phase_architecture_df(product):
    ...
    for i,s in enumerate(phase_list(product),1):
        rows.append({
            ...
            "Ara Ürün":(
                (intermediate_rule(product,s.get("title")) or {}).get("output","")
                or ", ".join(r["output"] for _,r in producer_for_consume_stage(product,s.get("title")))
                or "—"
            ),
...
/* v0.19 mobile dark-form contrast */
...
with head:
    ...
    st.caption(f"v0.20 MOBILE PRO • Kullanıcı: {USER['username']} • Rol: {ROLE}")
with logout:
    st.write("")
...
# ---------------- ADMIN UI ----------------
    ...
    if NAV=="🏷️ Etiketleme / QR":
        ...
        st.caption("v0.20: mobil okunabilir etiket motoru. Uyarı metinleri taslaktır; ticari satıştan önce ürünün SDS/SEA/CLP sınıflandırması ve hedef ülke etiket mevzuatıyla doğrulanmalıdır.")
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.20 MOBILE PRO — Portre formül kartları + gerçek yatay swipe • Eksik ara ürünü bulunduğun fazdan şimdi hazırla • WIP geri dönüş motoru • Yağ Fazı/Bakım/Solvent/Boya/Parfüm ara ürünleri • Audit.")
