        ...
        with c3:
            ...
            if actual>0 and not ok:
                ...
                with st.expander("🔐 REFERANS DIŞI DEĞER — DİJİTAL İMZA / YETKİLİ ONAYI",expanded=True):
                    ...
                    st.caption("Tolerans dışı dozaj için ADMIN dijital imzası zorunludur.")
                    au=st.text_input("Admin kullanıcı adı",value=USER["username"] if ROLE=="ADMIN" else "",
                                     key=f"oor_user_{active}_{sidx}_{midx}")
                    ap=st.text_input("Admin şifresi",type="password",key=f"oor_pass_{active}_{sidx}_{midx}")
                    if accept and override_reason.strip() and au.strip() and ap:
                        approved_user=verify_user_password(au.strip(),ap,required_role="ADMIN")
                        if approved_user:
                            st.success(f"✅ Dijital imza doğrulandı: {approved_user['username']} / ADMIN")
                            override=True
                        else:
                            st.warning("Admin kullanıcı adı / şifresi doğrulanmadı.")
            ...
            if state!="STAGE_HOLD":
                ...
                if rem>0:
                    ...
                    with st.expander("⚠️ Süre dolmadan tamamlandı olarak işaretle",expanded=False):
                        ...
                        sigpass=st.text_input("Kendi kullanıcı şifreniz (dijital imza)",type="password",
                                              key=f"holdsig_{active}_{sidx}")
                        signed=verify_user_password(USER["username"],sigpass) if sigpass else None
                        if signed:
            ...
            if not b.get("mix_started_at"):
                ...
                if rem>0:
                    ...
                    with st.expander("⚠️ Karışım erken tamamlandı — dijital imza ile devam",expanded=False):
                        ...
                        epass=st.text_input("Kendi kullanıcı şifreniz (dijital imza)",type="password",
                                            key=f"mixearly_pass_{active}_{sidx}")
                        early_signed=verify_user_password(USER["username"],epass) if epass else None
                        if early_signed:
                            st.success(f"İmza doğrulandı: {USER['username']} / {ROLE}")
...
def render_brand_header(login=False):
    """Compact corporate header optimized for mobile."""
    if LOGO_PATH.exists():
        if login:
            st.markdown("<div class='kc-mobile-logo'>",unsafe_allow_html=True)
            st.image(str(LOGO_PATH),use_container_width=True)
            st.markdown("</div>",unsafe_allow_html=True)
        else:
            a,b=st.columns([1.25,3.75],vertical_alignment="center")
            with a:
                st.image(str(LOGO_PATH),use_container_width=True)
            with b:
...
def _draw_tire_visual(draw,box,accent):
    ...
    r=min(x1-x0,y1-y0)//2
    # shadow / wheel
    draw.ellipse((cx-r,cy-r,cx+r,cy+r),fill=(20,24,28),outline=(90,100,108),width=3)
    inner=int(r*0.52)
    draw.ellipse((cx-inner,cy-inner,cx+inner,cy+inner),fill=(44,48,52),outline=_hex_rgb(accent),width=4)
    hub=int(r*0.15)
    draw.ellipse((cx-hub,cy-hub,cx+hub,cy+hub),fill=(10,13,16),outline=(160,170,175),width=2)
    # tread segments
    for dy in range(-r+12,r-10,16):
        draw.line((cx-r+8,cy+dy,cx-r+28,cy+dy+8),fill=(100,108,112),width=4)
        draw.line((cx+r-8,cy+dy,cx+r-28,cy+dy+8),fill=(100,108,112),width=4)
    draw.arc((cx-r-6,cy-r-6,cx+r+6,cy+r+6),200,330,fill=_hex_rgb(accent),width=6)

def generate_front_label_png(product,batch_no="",net_text="1 L"):
    en,tr,claim,accent=product_label_meta(product)
    codep=PRODUCTS.get(product,{}).get("code","")
    lot=batch_no or "KC-LOT-________"
    W,H=1240,1760
    img=Image.new("RGB",(W,H),(8,13,18))
    d=ImageDraw.Draw(img)
    white=(245,248,250); gray=(170,182,191); accent_rgb=_hex_rgb(accent)
    # safe top margin fixes logo cropping
    d.rounded_rectangle((45,45,W-45,H-45),radius=48,outline=(55,70,80),width=3,fill=(8,13,18))
    d.rounded_rectangle((90,82,W-90,98),radius=8,fill=accent_rgb)
    # Brand
    d.text((95,145),"Kaya",font=_font(88,True),fill=white)
    kaya_w=d.textbbox((95,145),"Kaya",font=_font(88,True))[2]-95
    d.text((95+kaya_w,145),"Car",font=_font(88,True),fill=accent_rgb)
    d.text((98,245),"PROFESSIONAL CAR CARE CHEMICALS",font=_font(28,False),fill=gray)
    d.line((95,310,W-95,310),fill=(65,80,90),width=2)
    # product visual
    if product in ("Lastik Parlatıcı Jel","Sıvı Lastik Parlatıcı"):
        _draw_tire_visual(d,(790,360,1120,690),accent)
    else:
        # generic chemistry/droplet visual
        d.ellipse((860,410,1040,590),outline=accent_rgb,width=10)
        d.polygon([(950,350),(875,455),(1025,455)],fill=accent_rgb)
    # product text
    d.text((95,390),en,font=_font(34,False),fill=gray)
    y=445
    for line in _wrap(d,tr,_font(62,True),650):
        d.text((95,y),line,font=_font(62,True),fill=white); y+=72
    y+=30
    for line in _wrap(d,claim,_font(31,True),1000):
        d.text((95,y),line,font=_font(31,True),fill=white); y+=42
    # icons / claims
    copy=label_copy(product)
    y=870
    for i,txt in enumerate(copy["icons"][:4]):
        x=95+(i%2)*520; yy=y+(i//2)*95
        d.rounded_rectangle((x,yy,x+470,yy+68),radius=20,outline=accent_rgb,width=2,fill=(15,23,30))
        d.text((x+22,yy+18),txt,font=_font(24,True),fill=white)
    # footer
    d.text((95,H-300),"PROFESSIONAL FORMULA",font=_font(34,True),fill=white)
    d.text((95,H-245),codep,font=_font(28,False),fill=gray)
    net_box=d.textbbox((0,0),net_text,font=_font(70,True))
    d.text((W-95-(net_box[2]-net_box[0]),H-300),net_text,font=_font(70,True),fill=white)
    d.text((95,H-145),f"BATCH / LOT: {lot}",font=_font(27,False),fill=gray)
...
def generate_back_label_png(product,batch_no="",net_text="1 L"):
    ...
    img=Image.new("RGB",(W,H),(8,13,18))
    d=ImageDraw.Draw(img)
    white=(245,248,250); gray=(175,185,193); accent_rgb=_hex_rgb(accent)
    d.rounded_rectangle((45,45,W-45,H-45),radius=48,outline=(55,70,80),width=3,fill=(8,13,18))
    d.rounded_rectangle((90,82,W-90,98),radius=8,fill=accent_rgb)
    d.text((95,130),"KayaCar",font=_font(60,True),fill=white)
    d.text((95,205),f"{tr} • {net_text}",font=_font(30,True),fill=gray)
    y=285
    sections=[
    ...
    for title,txt in sections:
        d.text((95,y),title,font=_font(27,True),fill=accent_rgb); y+=38
        lines=_wrap(d,txt,_font(23,False),1040)
        for line in lines:
            d.text((95,y),line,font=_font(23,False),fill=white); y+=31
        y+=24
        if y>1450: break
    d.line((95,H-230,W-95,H-230),fill=(65,80,90),width=2)
    d.text((95,H-205),f"Ürün Kodu / Product Code: {codep}",font=_font(23,False),fill=gray)
    d.text((95,H-170),f"Parti / Lot: {lot}",font=_font(23,False),fill=gray)
    d.text((95,H-135),"Üretim Tarihi: ____ / ____ / ______    SKT/TETT: ____ / ____ / ______",font=_font(21,False),fill=gray)
    d.text((95,H-100),"KayaCar Kimyasal • Professional Car Care Chemicals",font=_font(21,True),fill=white)
    bio=BytesIO(); img.save(bio,format="PNG",optimize=True); return bio.getvalue()

def generate_label_pdf(product,batch_no="",net_text="1 L"):
    front=Image.open(BytesIO(generate_front_label_png(product,batch_no,net_text))).convert("RGB")
    back=Image.open(BytesIO(generate_back_label_png(product,batch_no,net_text))).convert("RGB")
    bio=BytesIO()
    front.save(bio,format="PDF",save_all=True,append_images=[back],resolution=150.0)
    return bio.getvalue()
...
with head:
    ...
    st.caption(f"v0.12 MOBILE PRO • Kullanıcı: {USER['username']} • Rol: {ROLE}")
with logout:
    st.write("")
...
# ---------------- ADMIN UI ----------------
    ...
    with tabs[5]:
        ...
        with st.expander("➕ Tank / varil ekle",expanded=False):
            ...
            note=st.text_input("Tank notu / konum")
            if st.button("➕ TANK EKLE",type="primary",use_container_width=True):
                if cur>cap:
                    st.error("İlk miktar kapasiteden büyük olamaz.")
                else:
                    con=db()
                    con.execute("""INSERT INTO tanks(material,tank_name,capacity_kg,current_kg,mode,low_alarm_pct,active,note,updated_at)
                                   VALUES(?,?,?,?,?,?,?,?,?)""",
                                (tm,tn,cap,cur,"MANUEL",low,1,note,datetime.now().isoformat(timespec="seconds")))
                    # Manual tank creation with content must also be reflected in master stock.
                    if cur>0:
                    ...
                    con.commit(); con.close()
                    audit(USER["username"],"TANK_CREATE",f"{tn} | {tm} | {cur}/{cap} kg")
                    st.rerun()
        ...
        if tdf.empty:
            ...
            active_df=tdf[tdf["active"]==1] if "active" in tdf.columns else tdf
            ...
            for _,r in tdf.iterrows():
                ...
                with st.expander(f"{'🟢' if status=='AKTİF' else '⚪'} {r['tank_name']} • {r['material']} • {r['current_kg']:.2f}/{r['capacity_kg']:.2f} kg",expanded=False):
    ...
    # ---- Corporate Label / QR ----
    with tabs[8]:
        st.subheader("🏷️ Kurumsal Etiketleme / Ön + Arka Etiket / QR")
        st.caption("v0.12 çıktı motoru: PNG ön etiket + PNG arka etiket + 2 sayfalı PDF. Çok dilli metinler üretim taslağıdır; ticari baskıdan önce SDS/SEA/CLP ve hedef ülke mevzuatıyla doğrulanmalıdır.")
        ...
        front_png=generate_front_label_png(label_product,label_batch,pack)
        back_png=generate_back_label_png(label_product,label_batch,pack)
        pdf_bytes=generate_label_pdf(label_product,label_batch,pack)
        base=f"KayaCar_{PRODUCTS[label_product]['code']}_{label_batch or 'TASLAK'}"

        p1,p2=st.tabs(["⬛ ÖN ETİKET","📄 ARKA ETİKET"])
        with p1:
            st.image(front_png,caption="Ön etiket — baskı önizleme",use_container_width=True)
        with p2:
            st.image(back_png,caption="Arka etiket — TR / EN / DE",use_container_width=True)

        d1,d2,d3=st.columns(3)
        d1.download_button("⬇️ ÖN ETİKET PNG",front_png,file_name=f"{base}_ON.png",mime="image/png",use_container_width=True)
        d2.download_button("⬇️ ARKA ETİKET PNG",back_png,file_name=f"{base}_ARKA.png",mime="image/png",use_container_width=True)
        d3.download_button("⬇️ ÖN + ARKA PDF",pdf_bytes,file_name=f"{base}_ON_ARKA.pdf",mime="application/pdf",use_container_width=True)

        st.markdown("#### Etiket veri / QR")
        ...
        if BRAND_BOARD_PATH.exists():
            with st.expander("🎨 KayaCar marka sistemi / referans panosu"):
...
st.divider()
st.caption("KayaCar Kimyasal MES v0.12 MOBILE PRO — Faz üretim • Tank/Stok yönetimi • Dijital imza/override • Ön/arka etiket PNG/PDF • QR • Mobil arayüz.")
